...
- The PANORAMA public set lists five contributing institutions (RUMC, UMCG, ZGT, Karolinska,
  ...
  MSKCC. The grouping is decided from `clinical_information.xlsx` at split freeze. It is no
  longer an unwritten manual step: `make_splits.py` refuses a mapping that does not cover
  every raw value, and freezes whichever grouping was used into
  `config/frozen_thresholds.yaml`. `make_loso_splits.py` then enforces the pre-registration's
  escalation rule — it errors rather than emitting a leave-one-source-out fold whose held-out
  source is below a usable case count.
